...
class NumberCell():
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen,
                         NUMBER_CELL_COLOR,
                         (self.__x + NUMBER_CELL_BORDER_SIZE,
                          self.__y + NUMBER_CELL_BORDER_SIZE,
                          self.__width - (NUMBER_CELL_BORDER_SIZE * 2),
                          self.__height - (NUMBER_CELL_BORDER_SIZE * 2)),
                         0)

        if (self.__value != -1):
            label_font = pygame.font.SysFont('courier',
                                             NUMBER_CELL_FONT_SIZE,
                                             bold=True)
            label_text = label_font.render(str(self.__value),
                                           1,
                                           self.__get_font_color())
            label_x = ((self.__width / 2) -
                       (label_text.get_width() / 2) +
                       self.__x)
            label_y = ((self.__height / 2) -
                       (label_text.get_height() / 2) +
                       self.__y)
    ...
    def is_over(self, mouse_x, mouse_y):
        return ((mouse_x >= self.__x) and
                (mouse_x < (self.__x + self.__width)) and
                (mouse_y >= self.__y) and
                (mouse_y < (self.__y + self.__height)))
...
class Cell():
    ...
    def draw(self, screen, is_correct):
        ...
        def draw_right_line(cell_track_color):
            x1 = (self.__x +
                  CELL_BORDER_SIZE +
                  ((CELL_WIDTH - (2 * CELL_BORDER_SIZE)) / 2))
            y1 = (self.__y +
                  (CELL_HEIGHT / 2))
            x2 = (x1 +
                  ((CELL_WIDTH - (2 * CELL_BORDER_SIZE)) / 2))
...
class Label():
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen,
                         LABEL_COLOR,
                         (self.__x + LABEL_BORDER_SIZE,
                          self.__y + LABEL_BORDER_SIZE,
                          self.__width - (LABEL_BORDER_SIZE * 2),
                          self.__height - (BUTTON_BORDER_SIZE * 2)),
                         0)

        label_font = pygame.font.SysFont('courier',
                                         LABEL_FONT_SIZE,
                                         bold=True)
        label_text = label_font.render(self.__label,
                                       1,
                                       LABEL_FONT_COLOR)
        label_x = ((self.__width / 2) -
                   (label_text.get_width() / 2) +
...
class Button():
    ...
    def draw(self, screen):
        button_border_color = (BUTTON_ENABLED_BORDER_COLOR if self.__enabled
                               else BUTTON_DISABLED_BORDER_COLOR)
        button_color = (BUTTON_ENABLED_COLOR if self.__enabled
                        else BUTTON_DISABLED_COLOR)
        button_font_color = (BUTTON_ENABLED_LABEL_COLOR if self.__enabled
                             else BUTTON_DISABLED_LABEL_COLOR)
        ...
        pygame.draw.rect(screen,
                         button_color,
                         (self.__x + BUTTON_BORDER_SIZE,
                          self.__y + BUTTON_BORDER_SIZE,
                          self.__width - (BUTTON_BORDER_SIZE * 2),
                          self.__height - (BUTTON_BORDER_SIZE * 2)),
                         0)
